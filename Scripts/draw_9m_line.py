...
使い方 (Windows PowerShell):
    ...
    - left/right で別画像に個別描画する場合（両方指定可）:（1行で）
      ...
      複数行で書く場合は行末にバッククォート ` を使う（末尾に空白を置かないこと）:
        python Scripts/draw_9m_line.py `
        --left-image data/court_left.png `
        --left-output data/court_left_9m.png `
        --right-image data/court_right.png `
        --right-output data/court_right_9m.png
